...
class RevSliderOutput extends RevSliderFunctions {
	...
	public function get_image_data(){
		...
		if($bg_type != 'external'){
			...
			switch($slide->get_param(array('attributes', 'altOption'), $slide->get_param(array('attributes', 'titleOption')))){
				case 'media_library':
...
rs-module .material-icons {
  ...
  -moz-osx-font-smoothing: grayscale;
	...
	public function get_html_audio_data(){
		...
		$vl	= $this->get_val($layer, array('media', 'loop'), 'loopandnoslidestop');
		...
		if($vl !== 'loopandnoslidestop') $data['video']['l'] = $vl;
	...
	public function get_html_video_data(){
		...
		if(!in_array($videoloop, array('loopandnoslidestop', ''), true)) $data['video']['l'] = $videoloop;
	...
	public function add_html_background_video(){
		...
		$loop	= $slide->get_param(array('bg', 'video', 'loop'), 'none');
		...
		if(!in_array($loop, array('', 'none'), true)) $data['video']['l'] = $loop;
	...
	public function js_get_root(){
		...
		$keys = array(
			...
			'debugMode' => array(
				'v' => $s->get_param(array('troubleshooting', 'debugMode'), false),
				'd' => false
			),
			'fixedOnTop' => array(
