...
class Interactive_Promo extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'EA Interactive Promo', 'essential-addons-elementor' );
	}
   ...
   public function get_categories() {
	...
	protected function _register_controls() {
		...
		$this->add_group_control(
			...
			[
				'name' => 'promo_border',
				'selector' => '{{WRAPPER}} .eael-interactive-promo',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .eael-interactive-promo' => 'border-radius: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			'promo_overlay_color',
			[
				'label' => esc_html__( 'Promo Overlay Color', 'essential-addons-elementor' ),
				'type' => Controls_Manager::COLOR,
				'default' => '#3085a3',
				'selectors' => [
					'{{WRAPPER}} .eael-interactive-promo figure' => 'background-color: {{VALUE}};',
				],
			]
		);
