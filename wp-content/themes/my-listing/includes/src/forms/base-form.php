...
abstract class Base_Form {
	...
	public function display_recaptcha_field() {
		...
		$field['label'] = '';
		$field['required'] = true;
		$field['site_key'] = mylisting_get_setting( 'recaptcha_site_key' );
		mylisting_locate_template( 'templates/add-listing/form-fields/recaptcha-field.php', [
			'key'   => 'recaptcha',
			'field' => $field,
		] );
	}
	 ...
	 * @since 2.1
	 */
	public function validate_recaptcha_field( $fields ) {
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			throw new \Exception( _x( 'Security check failed. Please try again.', 'Add listing form', 'my-listing' ) );
		}
