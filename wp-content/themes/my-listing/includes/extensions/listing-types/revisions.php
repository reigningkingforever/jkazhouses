...
class Revisions {
	...
	public function add_revision( $type_id ) {
		$type = \MyListing\Ext\Listing_Types\Listing_Type::get( $type_id );
		if ( ! $type ) {
			return;
		}
	...
	public function rollback_revision() {
		...
		try {
			...
			$type = Listing_Type::get( $_GET['listing_type'] );
			if ( ! $type ) {
				throw new \Exception( __( 'Invalid request.', 'my-listing' ) );
			}
	...
	public function export_revision() {
		...
		try {
			...
			if ( $_GET['revision_id'] === 'current' ) {
				...
				$this->_force_download_json( $filename, $type->get_config() );
				exit;
			}
