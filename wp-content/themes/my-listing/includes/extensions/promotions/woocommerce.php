...
class WooCommerce {
	...
	public function add_promote_listing_action( $actions, $listing ) {
		...
		$package_id = mylisting()
							->promotions()
							->package()
							->get_listing_package( $listing->ID );

		if ( $package_id ) {
			$promote_link = [
				'type'    => 'plain',
				'content' => sprintf(
					'<li><a href="%s" class="listing-action-promoted">%s</a></li>',
					$this->package->get_edit_link( $package_id ),
					_x( 'Promoted', 'Promoted listing link name', 'my-listing' )
				),
			];

			$actions = [ 'promote' => $promote_link ] + $actions;
		} else {
			$listing = \MyListing\Src\Listing::get( $listing );

			$attrs = [];
			$attrs[] = 'class="listing-dashboard-action-promote"';
			$attrs[] = 'data-toggle="modal"';
			$attrs[] = 'data-target="#promo-modal"';
			$attrs[] = sprintf( 'data-listing-id="%s"', esc_attr( $listing->get_id() ) );
			$attrs[] = sprintf( 'data-listing-name="%s"', esc_attr( $listing->get_name() ) );
