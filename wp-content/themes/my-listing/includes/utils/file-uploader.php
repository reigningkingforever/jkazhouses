...
class File_Uploader {
	...
	public function upload( $file, $args ) {
		...
		if ( ! in_array( $file['type'], $args['allowed_mime_types'] ) ) {
			throw new \Exception( sprintf( _x( 'Uploaded files need to be one of the following file types: %s', 'File upload', 'my-listing' ), implode( ', ', array_keys( $args['allowed_mime_types'] ) ) ) );
		}
